...
export default function Login() {
  ...
  const [credHint, setCredHint] = useState({ user_email: 'user@gmail.com', asha_email: 'asha@healthmitra.local' })
  const { login } = useAuth()
  ...
  useEffect(() => {
    fetch('/api/auth/credentials-hint')
      .then(r => r.ok ? r.json() : null)
      .then(data => { if (data) setCredHint(data) })
      .catch(() => {})
  }, [])
  ...
  useEffect(() => {
    if (loginRole === 'asha') {
      setEmail(credHint.asha_email || 'asha@healthmitra.local')
    } else if (!rememberMe || !localStorage.getItem('hm_remembered_email')) {
      setEmail(credHint.user_email || '')
    }
  }, [loginRole, credHint.asha_email, credHint.user_email])
      ...
      <div className="auth-card animate-in">
        ...
        <form onSubmit={handleSubmit} className="auth-form">
          ...
          <div className="auth-field">
            ...
            <input type={showPass ? 'text' : 'password'} placeholder="Password" value={password} onChange={e => setPassword(e.target.value)} autoComplete="current-password" />
          ...
          <div className="auth-role-toggle" aria-label="Choose login type">
            <button type="button" className={`role-pill ${loginRole === 'user' ? 'active' : ''}`} onClick={() => setLoginRole('user')}>User</button>
            <button type="button" className={`role-pill ${loginRole === 'asha' ? 'active' : ''}`} onClick={() => setLoginRole('asha')}>ASHA Coordinator</button>
          </div>

          <button type="submit" className="btn btn-primary btn-lg auth-btn" disabled={loading}>
            {loading ? <><span className="spinner" style={{ width: 18, height: 18 }} /> Signing in...</> : <><LogIn size={18} /> {loginRole === 'asha' ? 'ASHA Login' : 'Login'}</>}
          </button>
        </form>

        <div className="auth-footer">
          {loginRole === 'asha' ? (
            <>ASHA login: <strong>{credHint.asha_email}</strong> · password in <strong>asha_credentials.txt</strong> (default: Asha@123)</>
          ) : (
            <>
              User login: <strong>{credHint.user_email}</strong> · password in <strong>user_credentials.txt</strong>
              <br />
